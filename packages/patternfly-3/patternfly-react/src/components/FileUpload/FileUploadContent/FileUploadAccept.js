...
const FileUploadAccept = ({ progressDone, acceptedFiles, rejectedFiles, text, icon, className, ...props }) => {
  const classes = classNames('upload-file-accept-text-pf', className);
  const rejectedCount = () => {
    if (rejectedFiles.length !== 1) return `${rejectedFiles.length} Files were rejected.`;
